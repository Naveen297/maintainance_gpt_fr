...
import { Eye, EyeOff, Mail, Lock, ArrowRight, Sparkles, Wrench, Settings, Cog, Zap, Shield, Monitor, Database, Factory } from 'lucide-react';
...
const LoginPage = ({ onLogin }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const response = await fetch('http://maintenancegpt.m-devsecops.com/clearsessions', {
        method: 'POST',
        headers: {
      ...
      <div className={`relative z-10 w-full max-w-md transform transition-all duration-1000 ${
        ...
        <div className="relative p-8 overflow-hidden border shadow-2xl rounded-3xl backdrop-blur-xl bg-slate-900/40 border-slate-700/50">
          ...
          <div className="relative z-10 space-y-5">
            ...
            {/* Enhanced plant selection dropdown */}
            <div className="relative group">
              <div className={`relative rounded-xl border transition-all duration-300 ${
                focusedInput === 'plant'
                  ? 'border-red-500/50 bg-slate-800/60 shadow-lg shadow-red-500/10'
                ...
                <Factory className={`absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 transition-all duration-300 pointer-events-none ${
                  focusedInput === 'plant' ? 'text-red-400' : 'text-slate-400'
                }`} />
                <select
                  value={plant}
                  onChange={(e) => setPlant(e.target.value)}
                  onFocus={() => setFocusedInput('plant')}
                  onBlur={() => setFocusedInput('')}
                  className="w-full py-4 pl-12 pr-4 text-sm text-white bg-transparent border-0 outline-none appearance-none cursor-pointer"
                  style={{
                ...
                >
                  <option value="" disabled className="bg-slate-800 text-slate-400">Select Plant</option>
                  <option value="Nashik Tool and Die" className="bg-slate-800 text-white">Nashik Tool and Die</option>
                  <option value="Chakan" className="bg-slate-800 text-white">Chakan</option>
                </select>
                {focusedInput === 'plant' && (
                  <div className="absolute inset-0 pointer-events-none bg-gradient-to-r rounded-xl from-red-500/5 to-red-600/5"></div>
                )}
            ...
            <button
              onClick={handleLogin}
              disabled={isLoading || !email.trim() || !password.trim() || !plant.trim()}
              className={`w-full py-4 rounded-xl font-semibold text-white transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] relative overflow-hidden group ${
                isLoading || !email.trim() || !password.trim() || !plant.trim()
