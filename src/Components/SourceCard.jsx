...
const SourceCard = ({ source, pageNo, score, text, onClick, isDark }) => {
  ...
  const isExcelFile = !!text && !pageNo; // Excel files have text but no pageNo
  
  const handleClick = () => {
    if (canPreview) {
      // For PDF files, call the API to get image
      onClick(source, pageNo);
    } else if (isExcelFile) {
      // For Excel files, pass the text content directly
      onClick(source, null, text);
    }
  };
