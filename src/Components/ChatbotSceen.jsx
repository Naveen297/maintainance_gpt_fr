...
import { Send, Moon, Sun, Mic, StopCircle, User, Bot, Settings, Bell, ChevronDown } from 'lucide-react';
...
const ChatbotScreen = ({ onLogout }) => {
    ...
    const [messages, setMessages] = useState([
      { id: 1, text: "Hello! I'm your AI assistant. How can I help you today?", sender: 'bot', timestamp: new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) }
    ]);
    ...
    return (
      <div className={`flex flex-col h-screen transition-all duration-300 ${
            ...
            <div className="relative">
              <button 
                ...
                className={`flex items-center space-x-2 p-2 rounded-lg transition-all duration-200 hover:scale-105 ${
                  isDark ? 'bg-gray-700/50 text-gray-300 hover:bg-gray-600/50' : 'bg-gray-100/80 text-gray-600 hover:bg-gray-200/80'
                }`}
              >
                ...
                <div className={`absolute right-0 mt-2 w-48 rounded-lg shadow-xl border transition-all duration-200 z-50 font-georama ${
                  ...
                  <div className="py-2">
                    <div className={`px-4 py-2 border-b ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
                      ...
                      {/* <div className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Admin</div> */}
                    </div>
          ...
          <div className="max-w-4xl px-6 py-6 mx-auto">
            ...
            <div className={`relative group ${
              ...
              <div className={`absolute inset-0 rounded-3xl transition-opacity duration-500 ${
                isDark 
                  ? 'bg-gradient-to-r from-red-500/5 via-transparent to-red-500/5 opacity-0 group-hover:opacity-100' 
                  : 'bg-gradient-to-r from-red-400/8 via-transparent to-red-400/8 opacity-0 group-hover:opacity-100'
              }`}></div>
                ...
                <div className="relative flex-1">
                  <textarea
                    ...
                    className={`w-full resize-none outline-none px-4 py-3 rounded-2xl min-h-[48px] max-h-[120px] transition-all duration-300 ${
                      isDark 
                        ? 'bg-gray-900/80 text-white placeholder-gray-400 border border-gray-700/50 focus:border-red-500/50 focus:bg-gray-900' 
                        : 'bg-white/90 text-gray-900 placeholder-gray-500 border border-gray-300/50 focus:border-red-400/50 focus:bg-white'
        ...
        <footer className={`px-6 py-3 text-center transition-all duration-300 ${
          isDark ? 'bg-gray-800 text-gray-500 border-t border-gray-700' : 'bg-white text-gray-400 border-t border-gray-200'
        }`}>
